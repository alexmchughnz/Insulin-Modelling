function [P, A, b, basisSplines] = FitSplinesxLnL(P, numKnots)
...
% Consider:
% dI/dt = c + cx*(1-xL) - cn*nL + kU*Uen - kI*I - kIQ*(I-Q)
%   with nL = sum(nLWeight_i * shape_i).

% Let cWeight_i = shape_i * cn.
% We can express equation as:
% dI/dt = k + cx*(1-xL) - sum(cWeight_i * nLWeight_i) - kI*I - kIQ*(I-Q)
cx = Uen/GC.VI;
cn = I./(1 + GC.alphaI*I);
...
% I(t) - I(t0) = int{k} + int{cx}*(1-xL) - int{sum(cWeight_i * nLWeight_i)} - kI*int{I} - kIQ*int{I-Q}
% Defining CX = -int{cx}, and CWeights = -int{cWeights}
% CX*xL + CWeights*nLWeights = I(t) - I(t0) + kI*int{I} + kIQ*int{I-Q} + CX - int{k} := C
...
I0 = I(1) * ones(size(I));
RHS = [I -I0 intITerm intIQTerm CX -intkTerm];
...
% [CX(t) CW_1(t) CW_2(t) ... CW_M(t)] * (xL; nLW1; nLW2; ... nLWM) = [C(t)] @ measurement times only
...
lb(1) = lbxL;
lb(2:numTotalParameters) = lbnLW;
ub(1) = ubxL;
ub(2:numTotalParameters) = ubnLW;
